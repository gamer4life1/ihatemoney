...
class Project(db.Model):
    ...
    def get_bills(self):
        ...
        return (Bill.query.join(
            Person, Project).filter(Bill.payer_id == Person.id).filter(
                Person.project_id == Project.id).filter(
                    Project.id == self.id).order_by(Bill.date.desc()).order_by(
                        Bill.creation_date.desc()).order_by(Bill.id.desc()))
    ...
    def get_member_bills(self, member_id):
        ...
        return (Bill.query.join(
            Person, Project).filter(Bill.payer_id == Person.id).filter(
                Person.project_id == Project.id).filter(
                    Person.id == member_id).filter(
                        Project.id == self.id).order_by(
                            Bill.date.desc()).order_by(Bill.id.desc()))
    ...
    def get_pretty_bills(self, export_format="json"):
        ...
        for bill in bills:
            ...
            pretty_bills.append({
                ...
                str(bill.date),
                "payer_name":
                Person.query.get(bill.payer_id).name,
                "payer_weight":
                Person.query.get(bill.payer_id).weight,
                "owers":
                owers,
            })
        return pretty_bills
...
class Person(db.Model):
    class PersonQuery(BaseQuery):
        def get_by_name(self, name, project):
            return (Person.query.filter(Person.name == name).filter(
                Project.id == project.id).one())

        def get(self, id, project=None):
            if not project:
                project = g.project
            return (Person.query.filter(Person.id == id).filter(
                Project.id == project.id).one())
    ...
    def has_bills(self):
        ...
        bills_as_ower_number = (db.session.query(billowers).filter(
            billowers.columns.get("person_id") == self.id).count())
...
class Bill(db.Model):
    ...
    def pay_each_default(self, amount):
        ...
        if self.owers:
            weights = (db.session.query(func.sum(Person.weight)).join(
                billowers, Bill).filter(Bill.id == self.id)).scalar()
    ...
    def __repr__(self):
        return (f"<Bill of {self.amount} from {self.payer} for "
